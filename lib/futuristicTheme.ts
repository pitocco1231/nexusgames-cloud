...
const PANELS: ThemePanel[] = [
  {
    ...
    description: [
      ...
      "**FLOW**　selecionar → validar → pedir → receber",
      "**DELIVERY**　canal privado / usuário correto",
      "",
      "### ◈ INICIAR",
      "`01` Acesse **🎮・PRODUTOS** e escolha sua plataforma.",
      "`02` Use o botão **COMPRAR** no painel do produto.",
      "`03` Preço, região e disponibilidade são validados antes da etapa final.",
      "`04` Use `/pedidos` para acompanhar seu histórico.",
      "",
      "🎟️ Precisou de ajuda? Use **🎟️・suporte**.",
      "🔐 Nunca envie keys, senhas ou dados de pagamento em canal público."
    ].join("\n")
  },
  {
    ...
    description: [
      "> Canal oficial de transmissões da **NexusGames**.",
      "",
      "`⚡` drops e ofertas",
      "`◈` novos produtos e plataformas",
      "`◇` atualizações do bot",
      "`◆` manutenções e avisos",
      "`✦` cupons e campanhas",
      "",
      "**VERIFY SOURCE** // confirme sempre pelo bot e pelos canais oficiais."
    ].join("\n")
  },
  {
    ...
    description: [
      "> Comprou na NexusGames? Registre sua experiência aqui.",
      "",
      "**FORMATO SUGERIDO**",
      "`PRODUTO` • `VELOCIDADE` • `ATENDIMENTO` • `NOTA`",
      "",
      "Exemplo: **Minecraft • rápido • atendimento ótimo • 5/5 ⭐**",
      "",
      "🔐 Não publique keys, QR Code, e-mail ou dados pessoais."
    ].join("\n")
  },
  {
    ...
    description: [
      "> Um fluxo único para reduzir erros e manter cada pedido organizado.",
      "",
      "`01` **ESCOLHA**　entre no canal da plataforma",
      "`02` **PRODUTO**　clique no botão do painel",
      "`03` **VALIDAÇÃO**　preço • região • disponibilidade",
      "`04` **PAGAMENTO**　somente após a validação do pedido",
      "`05` **CONFIRMAÇÃO**　o sistema confirma o status",
      "`06` **ENTREGA**　código enviado de forma privada",
      "`07` **HISTÓRICO**　acompanhe com `/pedidos`",
      "",
      "Use `/loja` para navegar ou `/comprar` para abrir o catálogo rápido."
    ].join("\n")
  },
  {
    channelNames: ["⚡・ofertas", "🔥・ofertas"], marker: "offers-v1", title: "⚡ NEXUS // DROPS",
    topic: "NEXUS // DROPS • ofertas temporárias, cupons e oportunidades.", image: "ofertas",
    description: [
      "> Ofertas e drops especiais da **NexusGames**.",
      "",
      "**DROP STATUS**　🟣 MONITORADO",
      "**REGRA**　confira plataforma • região • validade",
      "",
      "Quando um drop estiver ativo, todas as condições aparecem aqui antes da compra."
    ].join("\n")
  },
  {
    channelNames: ["🎟️・suporte", "🎫・suporte"], marker: "support-panel-v1", title: "🛰️ NEXUS // SUPPORT LINK",
    topic: "NEXUS // SUPPORT • abra um ticket privado para atendimento.", image: "suporte", buttonLabel: "ABRIR • TICKET",
    description: [
      "> Canal privado de suporte para compras, pagamentos, entrega e produtos.",
      "",
      "**SUPPORT STATUS**　🟣 ONLINE",
      "**PRIVACIDADE**　somente você + equipe NexusGames",
      "",
      "Tenha o número do pedido em mãos, caso já exista uma compra.",
      "",
...
function futuristicComponents(current: Array<Record<string, any>> | undefined, label?: string) {
  ...
  return current.map((row) => ({
    ...
    components: Array.isArray(row.components)
      ? row.components.map((component: Record<string, any>) => component.type === 2
        ? { ...component, style: 1, ...(label ? { label } : {}) }
        : component)
      : row.components
  }));
}
...
async function validBanner(name: string) {
  const url = `${STORE_URL}/assets/${name}?nexus-neon=20260910-v3`;
  const response = await fetch(url, { cache: "no-store", signal: AbortSignal.timeout(15000) });
  if (!response.ok) throw new Error(`Banner ${name} HTTP ${response.status}`);
  const type = response.headers.get("content-type") || "";
  const bytes = new Uint8Array(await response.arrayBuffer());
  if (!type.startsWith("image/jpeg") || bytes.length < 4 || bytes[0] !== 0xff || bytes[1] !== 0xd8) {
    throw new Error(`Banner ${name} nao retornou JPEG valido`);
  }
  return url;
}
...
export async function applyFuturisticTheme() {
  ...
  for (const panel of PANELS) {
    ...
    if (!channel) {
      results.push(`Canal nao encontrado: ${panel.channelNames[0]}`);
      continue;
    }

    if (channel.topic !== panel.topic) {
      await discordFetch(`/channels/${channel.id}`, { method: "PATCH", body: JSON.stringify({ topic: panel.topic }) });
    }
    ...
    for (const duplicate of candidates.slice(1)) {
      await discordFetch(`/channels/${channel.id}/messages/${duplicate.id}`, { method: "DELETE" }).catch(() => null);
    }

    for (const message of messages) {
      if (message.id === messageId || !message.author?.bot || !message.attachments?.length) continue;
      const isOldNexusAttachment = message.attachments.some((a) => /^(NexusGames-|nexusgames-panel-)/i.test(a.filename || ""));
      const hasOfficialMarker = message.embeds?.some((embed) => typeof embed.footer?.text === "string" && embed.footer.text.startsWith("NexusGames • canal:"));
      if (isOldNexusAttachment && !hasOfficialMarker) {
        await discordFetch(`/channels/${channel.id}/messages/${message.id}`, { method: "DELETE" }).catch(() => null);
      }
    }
